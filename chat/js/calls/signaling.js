...
function showIncomingPopup(call) {
  ...
  if (popup && callerEl && typeEl) {
    const callerName = state.rooms.find(r => r.other_user.id === call.caller_id)?.other_user?.full_name || "User";
    callerEl.textContent = callerName;
    typeEl.textContent = call.media_type === "video" ? "video call" : "voice call";
    popup.classList.remove("hidden");
    document.getElementById("acceptCallBtn").onclick = () => {
      popup.classList.add("hidden");
      import("./calls.js").then(m => m.acceptCall(call.id));
    };
    document.getElementById("declineCallBtn").onclick = () => {
      popup.classList.add("hidden");
      import("./calls.js").then(m => m.rejectCall(call.id));
    };
  }
}