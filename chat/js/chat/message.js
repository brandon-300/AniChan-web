...
export function renderMsgs(msgs, unreadIds = new Set()) {
  ...
  msgs.filter(m => !del.has(m.id)).forEach(msg => {
    ...
    if (!separatorInserted && unreadIds.has(msg.id)) {
      ...
      feed.appendChild(uSep);
      separatorInserted = true;
    }
    addMsg(msg, feed);
  });
  setTimeout(() => {
    const uSep = feed.querySelector(".unread-separator");
    if (uSep) {
...
document.getElementById("ctxEdit")?.addEventListener("click", () => {
  ...
  supabase.from("messages")
    ...
    .eq("id", state.ctxMsgId)
    .then(({ error }) => { if (error) alert("Edit failed"); else loadMsgs(state.activeRoom); });
});
document.getElementById("ctxDelMe")?.addEventListener("click", () => {
  $("ctxMenu").style.display = "none";
  addDel(state.ctxMsgId);
  loadMsgs(state.activeRoom);
});
document.getElementById("ctxDelAll")?.addEventListener("click", () => {
  $("ctxMenu").style.display = "none";
...
document.getElementById("ctxCancel")?.addEventListener("click", () => { $("ctxMenu").style.display = "none"; });
